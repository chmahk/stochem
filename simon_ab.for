...
C	O3+UCARB12->HOCH2CHO+CARB6+H2O2
...
C       NOA->CH3CO3+HCHO+NO2
        FLUX(JB,631)=FLUX(JB,631)+DJ(JB,31)*Y(JB,115)*DTS/M(JB)
C       NOA->CH3CO3+HCHO+NO2
        FLUX(JB,632)=FLUX(JB,632)+DJ(JB,32)*Y(JB,115)*DTS/M(JB)
...
C       RU14OOH->UCARB12+HO2+OH
        FLUX(JB,668)=FLUX(JB,668)+DJ(JB,68)*Y(JB,166)*DTS/M(JB)
C       RU14OOH->UCARB10+HCHO+HO2+OH
        FLUX(JB,669)=FLUX(JB,669)+DJ(JB,69)*Y(JB,166)*DTS/M(JB)
...
C     Reaction (61) O3 + C5H8 = UCARB10 + CO + HO2 + OH                                
         RC(JB,61) = 7.86D-15*EXP(-1913/TC(JB))*0.27 
C
C     Reaction (62) O3 + C5H8 = UCARB10 + HCOOH                                        
         RC(JB,62) = 7.86D-15*EXP(-1913/TC(JB))*0.73 
C
...
C     Reaction (362) O3 + UCARB10 = HCHO + CH3CO3 + CO + OH                             
         RC(JB,362) = 2.85D-18*0.59       
C
C     Reaction (363) O3 + UCARB10 = HCHO + CARB6 + H2O2                                 
         RC(JB,363) = 2.85D-18*0.41       
C
...
C     Reaction (374) O3 + UCARB12 = HOCH2CHO + CH3CO3 + CO + OH                         
         RC(JB,374) = 2.40D-17*0.89   
C
C     Reaction (375) O3 + UCARB12 = HOCH2CHO + CARB6 + H2O2                             
         RC(JB,375) = 2.40D-17*0.11   
C
...
C     Reaction (514) : OH + DMS = DMSO + HO2
        RC(JB,514)= 10.0E-5*(1.7D-42*EXP(7810/TC(JB))*O2(JB))/
...
c      DATA DVLAND(53),DVSEA(53)   /5.0,  5.0/ ! MVKOOH
      ...
      DATA DVLAND(220),DVSEA(220) /2.8,  0.0/ ! DMS (Judeikis et al. 1977)
...
C                                  Anth  ,Biomass,Veg   ,Soil  ,Oceans
      ...
      DATA (CLASS(IC,220),IC=1,5)  /0.13   ,0.0  ,3.18   ,0.41  ,28.1/ ! DMS 
...
C DMS
     ...
     &      CLASS(5,220)*OCEAN(I1,J1))*1E12*NA/(62.0*31536000.0)
C CH3BR
